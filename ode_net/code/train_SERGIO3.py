...
parser.add_argument('--settings', type=str, default='config_SERGIO2.cfg')
clean_name =  "SERGIO_data2" 
...
if __name__ == "__main__":
    ...
    # Initialization
    odenet = ODENet2(device, data_handler.dim, explicit_time=settings['explicit_time'], neurons = settings['neurons_per_layer'])
    odenet.float()
    ...
    with open('{}/network.txt'.format(output_root_dir), 'w') as net_file:
        ...
        net_file.write('\n\n\n')
        net_file.write(inspect.getsource(ODENet2.forward))
        net_file.write('\n')
        if abs_prior:
...
#       opt = optim.Adam(odenet.parameters(), lr=settings['init_lr'], weight_decay=settings['weight_decay'])
    ...
    extract_dynamics_matrix(output_root_dir, val=True)

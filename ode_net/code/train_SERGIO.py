...
def plot_MSE(epoch_so_far, training_loss, validation_loss, prior_losses, img_save_dir):
    ...
    np.savetxt('{}full_loss_info.csv'.format(output_root_dir), np.c_[training_loss, validation_loss], delimiter=',')
...
def read_prior_matrix(prior_mat_file_loc, sparse = False, num_genes = 100):
    if sparse == False: 
...
def validation(odenet, val_data, val_time_pt, method):
    with torch.no_grad():
        predictions = torch.zeros(val_data.shape).float()
        targets = torch.zeros(val_data.shape).float()

        data = torch.from_numpy(val_data).float()
        for i in range(len(val_time_pt)-1):
            start_data = data[:,i].unsqueeze(0)
            if (val_time_pt[i+1] - val_time_pt[i] > 0.00001):
                time = torch.tensor([val_time_pt[i], val_time_pt[i+1]])
                predictions[:,i] = odeint(odenet, start_data, time, method=method, atol=0.0001)[1]
                targets[:,i] = data[:,i+1]
...
def split_data(data_file_path, validation_split):
    raw = np.loadtxt(data_file_path, delimiter=',')
    num_genes = raw.shape[0] - 1 #Last row is time points
    num_time_pts = raw.shape[1]
    time_pts = raw[-1, :]
    data = raw[:-1,:]
    num_time_pts_val_set = ceil(validation_split * num_time_pts)
    num_time_pts_train_set = num_time_pts - num_time_pts_val_set

    #Make sure validation set contains only unique pseudo-time points
    val_time_pt = []
    val_time_pt_indices = []
    indices = np.random.permutation(num_time_pts)
    count = 0
    for i in range(len(indices)):
        time_pt = time_pts[i]
        if not (time_pt in val_time_pt):
            val_time_pt.append(time_pt)
            val_time_pt_indices.append(i)
            count += 1
            if (count >= num_time_pts_val_set):
                break
    
    #Sort validation time_points based on pseudo-time order
    combined = list(zip(val_time_pt, val_time_pt_indices))
    sorted_combined = sorted(combined, key=lambda x: x[0])
    val_time_pt, val_time_pt_indices = zip(*sorted_combined)
    val_time_pt = np.array(val_time_pt)
    val_data = np.zeros((num_genes, num_time_pts_val_set))
    for i in range(len(val_time_pt_indices)):
        val_data[:,i] = data[:,val_time_pt_indices[i]]

    val_time_pt_indices_set = set(val_time_pt_indices)
    train_time_pt = np.zeros(num_time_pts_train_set)
    train_data = np.zeros((num_genes, num_time_pts_train_set))
    count = 0
    for i in range(num_time_pts):
        if (i in val_time_pt_indices_set):
            continue
        else:
            train_time_pt[count] = time_pts[i]
            train_data[:,count] = data[:,i]
            count += 1

    # Data points are unordered and can be at same pseudo-time point, so map is needed
    next_time_pt_map = {}
    sorted_train_time_pts = np.unique(train_time_pt)
    num_unique_time_pts = len(sorted_train_time_pts)
    for i in range(num_time_pts_train_set):
        time_pt = train_time_pt[i]
        time_pt_index = np.where(sorted_train_time_pts == time_pt)[0][0]
        if (time_pt_index + 1 < len(sorted_train_time_pts)):
            next_time_pt = sorted_train_time_pts[time_pt_index+1]
            next_time_pt_index = np.where(train_time_pt == next_time_pt)[0][0]
            next_time_pt_map[i] = next_time_pt_index
        else:
            next_time_pt_map[i] = -1

    return train_data, train_time_pt, val_data, val_time_pt, next_time_pt_map

def generate_batch_indices_for_epoch(num_time_pts, batch_size):
    indices = list(range(num_time_pts))
    ...
    for i in range(num_time_pts):
        if (num_in_batch >= batch_size):
            ...
            cur_batch.append(indices[i])
            num_in_batch += 1

    return batch_indices 

def training_step(odenet, batch_indices, train_data, train_time_pt, next_time_pt_map, opt, method, batch_size, explicit_time, relative_error, batch_for_prior, prior_grad, loss_lambda, device):
    opt.zero_grad()
    predictions = torch.zeros((train_data.shape[0], len(batch_indices))).float().to(device)
    target = torch.zeros((train_data.shape[0], len(batch_indices))).float().to(device)
    batch_data = train_data[:,batch_indices].copy()
    batch_data = torch.from_numpy(batch_data).float().to(device)
    for i in range(len(batch_indices)):
        start_gene_data = batch_data[:,i].unsqueeze(0)
        start_time = train_time_pt[batch_indices[i]].item()
        end_time_pt_idx = next_time_pt_map[batch_indices[i]]
        if (end_time_pt_idx >= 0): #Otherwise, at last time point
            end_time = train_time_pt[end_time_pt_idx].item()
            if (end_time - start_time) > (0.0001):
                time = torch.tensor([start_time, end_time])
                target[:,i] = torch.from_numpy(train_data[:,end_time_pt_idx])
                predictions[:,i] = odeint(odenet, start_gene_data, time, method=method, atol=0.0001)[1]
    
    loss_data = torch.mean((predictions - target)**2)
    
    t = torch.tensor(train_time_pt[batch_indices])
...
parser.add_argument('--settings', type=str, default='config_SERGIO.cfg')
clean_name =  "SERGIO_data" 
parser.add_argument('--data', type=str, default='../../SERGIO_data/{}.csv'.format(clean_name))
...
if __name__ == "__main__":
    ...
    train_data, train_time_pt, val_data, val_time_pt, next_time_pt_map = split_data(args.data, settings['val_split'])

    abs_prior = True
    
    #Read in the prior matrix
    prior_mat_loc = '../../SERGIO_data/adjacency_matrix_10_edges.csv'
    prior_mat = read_prior_matrix(prior_mat_loc, sparse = False, num_genes = train_data.shape[0])
    
    if abs_prior:
        prior_mat = torch.abs(prior_mat)

    K = 10000
    batch_for_prior = (torch.rand(K,1,prior_mat.shape[0], device = device)- 0.5)*1
    ...
    # Initialization
    odenet = ODENet(device, train_data.shape[0], explicit_time=settings['explicit_time'], neurons = settings['neurons_per_layer'])
    odenet.float()
    param_count = sum(p.numel() for p in odenet.parameters() if p.requires_grad)
    param_ratio = round(param_count/ (train_data.shape[0])**2, 3)
...
#       opt = optim.Adam(odenet.parameters(), lr=settings['init_lr'], weight_decay=settings['weight_decay'])
    ...
    for epoch in range(1, tot_epochs + 1):
        ...
        batch_indices = generate_batch_indices_for_epoch(train_data.shape[1], settings['batch_size'])
        iterations_in_epoch = len(batch_indices)
        ...
        for batch in batch_indices:
            loss_list = training_step(odenet, batch, train_data, train_time_pt, next_time_pt_map, opt, settings['method'], settings['batch_size'], settings['explicit_time'], settings['relative_error'], batch_for_prior, prior_grad, loss_lambda, device)
            loss = loss_list[0]
        ...
        val_loss = validation(odenet, val_data, val_time_pt, settings['method']).item()
        ...
        print("Validation loss {:.5E}, using {} points".format(val_loss, val_loss))
        ...
        if (epoch in rep_epochs) or (consec_epochs_failed == epochs_to_fail_to_terminate):
            ...
            plot_MSE(epoch, training_loss, validation_loss, prior_losses, img_save_dir)    
            ...
            L = [rep_epochs_so_far, rep_epochs_time_so_far, rep_epochs_train_losses, rep_epochs_val_losses]
    ...
    validation_network_path = '../../SERGIO_data/validation_network.csv'
    genes_path = '../../SERGIO_data/gene_names.csv'
